(ns toolmaker.actor
  "ToolmakerActor — the ISCO-08 7222 toolmaking workshop
  ...
  The unconditional invariant: the Toolmaker Advisor can never
  directly commit an operating record the ToolmakerGovernor refuses,
  and can never make this actor finalize a machining-execution
  decision (e.g. a specific lathe, mill or grinder operation) or
  ...
  coordinates WORKSHOP SCHEDULING/LOGISTICS ONLY — it never operates
  precision machine tools or authorizes machining work itself."
...
(defn build-graph
  "Build a compiled ToolmakerActor graph. `store` implements
  `toolmaker.store/Store`. `advisor` implements
  `toolmaker.advisor/Advisor` (defaults to `mock-advisor`).
