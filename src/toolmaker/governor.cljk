(ns toolmaker.governor
  "ToolmakerGovernor — the independent safety/scope layer gating every
  workshop scheduling/logistics proposal an advisor may make for a
  toolmaking crew. The governor never dispatches hardware itself,
  never operates precision machine tools (lathes, mills, grinders) on
  the shop floor, and never finalizes a machining-execution decision
  (e.g. deciding to proceed with a specific lathe, mill or grinder
  operation) or overrides a shop safety officer's judgment — those
  ...
  performs machining work itself). Modeled on cloud-itonami-isco-7111's
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never operates a machine tool itself;
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                machining-execution decision (e.g.
                                deciding to proceed with a specific
                                lathe, mill or grinder operation), or
                                ...
                                machining operation\" / \"authorize
                                the machining operation\" / \"override
                                the shop safety officer's judgment\",
                                never as bare nouns like \"machining\"
                                ...
                                crew operation for machining task …\"
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a machinery-hazard / tool-condition
...
(def ^:private scope-excluded-ops
  #{:finalize-machining-decision :authorize-machining-operation
    :proceed-with-machining-operation :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("machining", "lathe", "mill", "grinder", "safety", "shop safety
...
;; nouns, e.g. "machining task" / "shop safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the machining operation" "proceed with the machining work"
   "finalize the machining decision" "authorize the machining operation"
   "authorize the machining operation decision"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :no-actuation
             :detail "effect は :propose のみ許可（governor は工場作業を直接実行しない）"})
      ...
      (conj {:rule :scope-excluded-action
             :detail "機械加工の実行判断（旋盤/フライス/研削等）の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `toolmaker.store/Store`. Pure — never mutates
