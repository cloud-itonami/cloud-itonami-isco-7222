(ns toolmaker.advisor
  "Toolmaker Advisor — proposing a toolmaking workshop scheduling/logistics
  coordination operation (log a work record, schedule a crew
  operation, flag a safety concern, coordinate a tooling-materials
  supply order) from a crew roster, workshop registration and
  safety-reporting policy. Swappable mock/llm; the advisor ONLY
  proposes — `toolmaker.governor` independently gates every proposal
  and always escalates safety concerns and above-threshold supply
  orders. The advisor never proposes to directly finalize a
  machining-execution decision (e.g. a specific lathe, mill or
  grinder operation) or to override a shop safety officer's judgment
  — those stay permanently out of this actor's scope. Modeled on
  cloud-itonami-isco-7111's advisor for the physical-safety-domain
  shape.
...
(defn- rationale-for [op worker-id workshop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for machining task at workshop " workshop-id)
...
(def ^:private system-prompt
  "You are a toolmaking workshop scheduling/logistics coordination
   ...
   propose to directly finalize a machining-execution decision (e.g.
   deciding to proceed with a specific lathe, mill or grinder
   operation), or to override a shop safety officer's judgment —
   those are always out of this actor's scope; it coordinates
   workshop scheduling/logistics only and never operates precision
   machine tools or authorizes machining work itself. Safety concerns
