(ns toolmaker.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [toolmaker.actor :as actor]
...
(deftest commits-a-registered-work-log
  (let [st (fresh-store)
        ...
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :workshop-id "WS-1" :task "die-cutting progress log"}
...
(deftest holds-an-unregistered-workshop-proposal
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:worker-id "worker-1" :op :log-work-record :stake :low
                  :workshop-id "WS-ghost" :task "die-cutting progress log"}
...
(deftest holds-a-scope-excluded-op-even-at-high-confidence
  (testing "an actor run can never commit a proposal that would finalize a machining-execution decision, regardless of disposition path"
    (let [st (fresh-store)
          graph (actor/build-graph {:store st})
          request {:worker-id "worker-1" :op :finalize-machining-decision :stake :low
                    :workshop-id "WS-1" :task "machining decision"}
